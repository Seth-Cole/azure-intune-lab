...
## Purpose
Establishes baseline security posture for all Windows devices accessing network resources, reducing attack surface through automated compliance assessment. If devices are out of compliance an automated message is sent to user which includes compliance issues monitored and inform them of expected remediation time prior to next action.
